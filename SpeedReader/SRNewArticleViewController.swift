...
//        print("clicked")
        ...
        openPanel.allowedFileTypes = ["txt","rtf","rtfd","doc", "docx", "html", "webArchive"]
...
//        print("Opened \(String(describing: fileURL))")
        ...
        let article: Article = Article(context: context)
        article.typeOfArticle = 1
        article.content = "Imported from File"
        
        var parseAttribute: String = convertFromNSAttributedStringDocumentType(NSAttributedString.DocumentType.plain)
        
        if normalizedExtension == "rtf" {
            parseAttribute = convertFromNSAttributedStringDocumentType(NSAttributedString.DocumentType.rtf)
        } else if normalizedExtension == "doc" {
            parseAttribute = convertFromNSAttributedStringDocumentType(NSAttributedString.DocumentType.docFormat)
        } else if normalizedExtension == "docx" {
            parseAttribute = convertFromNSAttributedStringDocumentType(NSAttributedString.DocumentType.wordML)
        } else if normalizedExtension == "html"{
            parseAttribute = convertFromNSAttributedStringDocumentType(NSAttributedString.DocumentType.html)
        } else if normalizedExtension == "webArchive" {
            parseAttribute = convertFromNSAttributedStringDocumentType(NSAttributedString.DocumentType.webArchive)
        }
        do {
            let attributedStringWithRtf:NSAttributedString = try NSAttributedString(url: fileURL, options: convertToNSAttributedStringDocumentReadingOptionKeyDictionary([convertFromNSAttributedStringDocumentAttributeKey(NSAttributedString.DocumentAttributeKey.documentType):parseAttribute]), documentAttributes: nil)
            article.content = attributedStringWithRtf.string
        } catch {}
        article.lastUpdated = Date.init()
...
//        print("clicked")
        if let webWindow = (storyboard?.instantiateController(withIdentifier: "OpenWebWindow") as? NSWindowController)?.window {
            NSApp.mainWindow?.beginSheet(webWindow, completionHandler: { (response: NSApplication.ModalResponse) in
//                print("sheet closed")
